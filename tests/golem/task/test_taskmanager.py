...
class TestCopySubtaskResults(TwistedTestCase):
    ...
    def test_copy_subtask_results(self):  # pylint: disable=too-many-locals
        ...
        zip_mock = zip_patch.start()
        os_mock = os_patch.start()
        shutil_mock = shutil_patch.start()
        self.addCleanup(zip_patch.stop)
        self.addCleanup(os_patch.stop)
        self.addCleanup(shutil_patch.stop)

        zip_mock.return_value.__enter__().namelist.return_value = [
            'stdout',
            'stderr',
            'result',
            '.package_desc'
        ...
        def verify(_):
            ...
            shutil_mock.copy.assert_called_once_with(old_zip_path, new_zip_path)
            os_mock.makedirs.assert_called_once_with(extract_path)
            zip_mock.assert_called_once_with(new_zip_path, 'r')
            zip_mock().__enter__().extractall.assert_called_once_with(
                extract_path)
